...
export class DeviceController {
  ...
  @ApiQuery({ name: 'pageNumber', description: 'Page number' })
  @ApiResponse({
    status: HttpStatus.OK,
    type: [DeviceDTO],
  ...
  async getMyDevices(
    ...
    @Query(ValidationPipe) filterDto: OrgFilterDTO,
    @UserDecorator() { organizationId }: ILoggedInUser,
  ) {
  ...
  @Put('/my/deviceCreatedAt')
  @UseGuards(AuthGuard('jwt'), PermissionGuard)
  @Permission('Write')
  @ACLModules('DEVICE_MANAGEMENT_CRUDL')
  @ApiResponse({
    status: HttpStatus.OK,
    description: "change the device's createdAt date",
  })

  @ApiQuery({ name: 'deviceId', description: 'Device Id' })
  @ApiQuery({ name: 'givenDate', description: 'Update the createdAt date', type: Date, })
  async changecreatedAt(
    @UserDecorator() { organizationId }: ILoggedInUser,
